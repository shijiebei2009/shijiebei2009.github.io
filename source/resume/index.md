...
## 个人信息
 ...
 - 籍贯：江苏省徐州市睢宁县
 - 户口：上海市
 - 期望职位：① Elasticsearch/Lucene搜索工程师 ② Java工程师 ③ Python工程师
 - 期望薪资：面谈
 - 期望城市：上海、杭州、南京
...
- 集团操作日志项目，操作日志主要记录了Android、IOS、编辑后台、作家后台、Job任务调度等操作记录，早期版本数据使用MySQL存储，但是无法解决跨库跨表分页及排序的难题，预估数据量在1G/天，存量记录3亿条左右，数据量230G左右，将存量及增量数据入Elasticsearch集群，使用Elasticsearch集群对外提供查询服务
- 基于Canal和MySQL binlog的日志同步系统
